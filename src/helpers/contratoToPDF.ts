...
export default class ContratoToPDF {
    ...
    static async  getContratoPDF(contrato: any) {
        ...
        doc.text(`CONTRATO DE MUTUO DINERARIO N° “CODIGO DE LA PERSONA ${contrato.prestamista.prestamista_codigo} - ${contrato.contrato_numero}`, { align: 'center', underline: true });
        doc.text(`
        `);
        doc.text(`Conste por el presente contrato de mutuo dinerario que celebran de una parte ”NOMBRE DE LA PERSONA”, identificado con D.N.I. Nº “DNI DE LA PERSONA”, con domicilio real en “DOMICILIO DE LA PERSONA”, de la Provincia y Departamento de “DEPARTAMENTO DE LA PERSONA”, a quien en adelante se le denominará EL MUTUANTE y de la otra parte CORPORACIÓN VICAM identificado con RUC. N° 20602294979, debidamente representado Sergio Enrique Chung Chung en calidad de gerente general, identificado con DNI. Nº 46097329 y con domicilio en Calle Eugenio de la Torre Nro. 231 Dpto. 502 distrito de San Miguel de la Provincia y Departamento de Lima, a quien en adelante se le denominará EL MUTUATARIO; en los términos y condiciones siguientes:
        `, { align: 'justify' });
        ...
        doc.font('Helvetica').text(`En virtud del presente contrato, EL MUTUANTE otorgó en calidad de préstamo con intereses a favor de EL MUTUATARIO la suma de S/. “MONTO DE PRESTAMO DE LA PERSONA” (“CANTIDAD DE PRESTAMO ESCRITO DE LA PERSONA” soles), los mismos que fueron depositados el día “FECHA DE DEPOSITO: DIA de MES del AÑO”, como consta en la última hoja del presente contrato.
            `, { align: 'justify' });
        ...
        doc.font('Helvetica').text(`EL MUTUATARIO se compromete a pagar la deuda capital y los intereses a EL MUTUANTE en el plazo improrrogable de “DIAS DE PRESTAMO” días calendario, computados a partir del día “DIA SIGUIENTE DEL PRESTAMO: DIA de MES del AÑO”, hasta el día “DIA FINAL DEL PRESTAMO: DIA de MES del AÑO”.
            `, { align: 'justify' });
        doc.font('Helvetica').text(`Al cumplirse dicho plazo, la suma total a pagar es de S/. “MONTO DE PRESTAMO MAS INTERESES DE LA PERSONA” (“CANTIDAD DE PRESTAMO MAS INTERESES ESCRITO DE LA PERSONA” soles)
            `);
        ...
        doc.font('Helvetica').text(`Las partes dejan constancia que la devolución del capital y pago de intereses se realizará a EL MUTUANTE en calidad de transferencia a:
            `, { align: 'justify' });
        doc.font('Helvetica').text(`CUENTA DE AHORROS “NUMERO DE CUENTA DE LA PERSONA” “BANCO DE LA PERSONA” , o cheque si este lo considere necesario.
            `, { align: 'justify' });
        doc.font('Helvetica-Bold').text(`SEXTO: DEL PLAZO`);
        doc.font('Helvetica').text(`El plazo en que se cancelará la deuda incluyendo el interés pactado por parte de EL MUTUATARIO es de “DIAS DE PRESTAMO” (“DIAS DE PRESTAMO ESCRITOS”) días contabilizados al día siguiente de realizado el abono, en la forma y lugar señalado en las cláusulas anteriores.
            `, { align: 'justify' });
        ...
        doc.font('Helvetica').text(`                  * MUTUANTE: ”NOMBRE DE LA PERSONA”/ ”CORREO DE LA PERSONA”
        `);
        doc.addPage();
        doc.image(logo,(doc.page.width - 180) /2,50,{ width: 180 });
        doc.text(`
        ...
        doc.font('Helvetica').text(`Ambas partes están conformes con el contenido del presente contrato de mutuo en fe de lo cual legalizan sus respectivas firmas.
            `, { align: 'justify' });
        doc.font('Helvetica').text(`Lima`,{align:'right'});
        doc.text(``);
        doc.text(``);
        doc.text(``);
        doc.end();
